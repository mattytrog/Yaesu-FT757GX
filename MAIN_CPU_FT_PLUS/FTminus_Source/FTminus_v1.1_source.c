...
{
return read_eeprom(base);
}

void beep();
void write8(int8 base, int8 val)
{
int8 res = read_eeprom(base);
if(res == val) return;
beep();
write_eeprom(base, val);
}
...
int8 loc = (base * 5);
   dmhz = ram_store2[loc];
   d100k = ram_store2[loc+1];
   d10k = ram_store2[loc+2];
   d1k = ram_store2[loc+3];
   d100 = ram_store2[loc+4];
#else
int8 loc = 0xB0 + (base * 5);
   dmhz = read8(loc);
   d100k = read8(loc+1);
   d10k = read8(loc+2);
   d1k = read8(loc+3);
...
int8 loc = (base * 5);
   ram_store2[loc] = dmhz;
   ram_store2[loc+1] = d100k;
   ram_store2[loc+2] = d10k;
   ram_store2[loc+3] = d1k;
   ram_store2[loc+4] = d100;
#else
   int8 loc = 0xB0 + (base * 5);
   write8(loc, dmhz);
   write8(loc + 1, d100k);
...
int8 loc = (base * 5);
   dmhz = ram_store[loc];
   d100k = ram_store[loc+1];
   d10k = ram_store[loc+2];
   d1k = ram_store[loc+3];
   d100 = ram_store[loc+4];
#else
int8 loc = 0x00 + (base * 5);
   dmhz = read8(loc);
   d100k = read8(loc+1);
   d10k = read8(loc+2);
   d1k = read8(loc+3);
...
int8 loc = (base * 5);
   ram_store[loc] = dmhz;
   ram_store[loc+1] = d100k;
   ram_store[loc+2] = d10k;
   ram_store[loc+3] = d1k;
   ram_store[loc+4] = d100;
#else
   int8 loc = 0x00 + (base * 5);
   write8(loc, dmhz);
   write8(loc + 1, d100k);
...
int8 loc = (base * 5);
   dmhz = ram_store1[loc];
   d100k = ram_store1[loc+1];
   d10k = ram_store1[loc+2];
   d1k = ram_store1[loc+3];
   d100 = ram_store1[loc+4];
#else
int8 loc = 0x40 + (base * 5);
   dmhz = read8(loc);
   d100k = read8(loc+1);
   d10k = read8(loc+2);
   d1k = read8(loc+3);
...
int8 loc = (base * 5);
   ram_store1[loc] = dmhz;
   ram_store1[loc+1] = d100k;
   ram_store1[loc+2] = d10k;
   ram_store1[loc+3] = d1k;
   ram_store1[loc+4] = d100;
#else
   int8 loc = 0x40 + (base * 5);
   write8(loc, dmhz);
   write8(loc + 1, d100k);
...
int8 buttonres(INT8 res)
{
   ...
   settings[0] = active_vfo;
   settings[1] = band;
   settings[2] = mr_mode;
   settings[3] = mem_channel;
   settings[4] = dcs;

#ifdef use_eeprom
   for (int i = 0; i < 5; ++i)
   {
   write8 ((0xF0 + i), settings[i]);
   }
#endif
   RETURN res;
}
...
void values()
{
   ...
   for (int i = 0; i < 5; ++i)
   {
   settings[i] = read8 (0xF0 + i);
   }
   active_vfo =   settings[0];
   band =         settings[1];
   mr_mode =      settings[2];
   mem_channel =  settings[3];
   dcs =          settings[4];
   set_dcs(dcs);
   load_vfo(active_vfo);
   }
...
void main()
{
   ...
   WHILE(true)
   {
      ...
      IF( ! pms)
      {
         ...
         if(!dial_moved())
         {
         if(btnrefresh < (refresh + 1)) ++btnrefresh; 
         }
         ...
         if(counter > 32768) {save_vfo(active_vfo); counter = 0;}
