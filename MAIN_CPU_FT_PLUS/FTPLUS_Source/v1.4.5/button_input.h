...
void vfoab()
{
   INT8 state = get_state (1);
   if(state == 4) {save8(cb_reg_n,cb_region); save8(cb_ch_n, cb_channel);set_state(active_vfo - 1); }
...
#ifdef include_cb
   IF (state == 4) save_band_vfo_f (active_vfo, band, frequency) ;
#endif
   blink();
   read_all_state();
}
...
void btn_up(int8 state)
{
   if(!state) state = get_state(0);
   if(sw_500k) state +=4;
   if((state == 1) || (state == 2))
   {
...
void toggle_cb_mode()
{
   if((gen_tx) && (!cb_disabled))
   {
      ...
      IF(state == 4)
      {
         ...
         mem_mode = 0; save8(mode_n,mem_mode);
         read_all_state();
         update_PLL(frequency);
      }
   } else errorbeep(3);
...
void LONG_up()
{
   ...
   IF(state  == 3) {IF(band  == 9)band = 0; ELSE ++band; frequency = load_band_vfo_f(active_vfo, band);}
   blink();
}
...
void LONG_dn()
{
   ...
   IF(state  == 3) {IF(band  == 0)band = 9; ELSE --band; frequency = load_band_vfo_f(active_vfo, band);}
   blink();
}
...
{
         int8 state = get_state(0);
         IF(state  < 3)frequency += increment;
         IF(state  == 3)btn_up(3);
         IF(state  == 4)btn_up(4);
...
int8 buttonaction (INT8 opt, int8 increment)
{
   ...
   SWITCH(opt)
   {
      ...
      CASE 2: beep(); btn_dn(0); break;
      CASE 3: beep(); btn_up(0); break;
      CASE 4: mvfo(); break; //MVFO
      CASE 5: beep(); vfoab(); break; //VFOAB
      CASE 6: beep(); dial_lock_button(); break;
      CASE 7: vfom(); break; //VFOM
      CASE 8: beep(); mrvfo(); break; //MRVFO
      CASE 9: beep(); split_button(); break;
      CASE 10: vfom_swap(); break; //VFOM SWAP
...
int8 res = 0;
         ...
         IF(pb2) res = 1;//(RESULT: 1)Clarifier
         IF(pb1) res = 2;//(RESULT: 2)Down
         IF(pb0) res = 3;//(RESULT: 3)Up
         
         k2 = 0; k4 = 1; delay_us(ondelay);
         IF(pb2) res = 4;//(RESULT: 4)M > VFO
         IF(pb1) res = 5;//(RESULT: 5)VFO A / B
         IF(pb0) res = 6;//(RESULT: 6)Dial lock
         
         k4 = 0; k8 = 1; delay_us(ondelay);
         IF(pb2) res = 7;//(RESULT: 7)VFO > M
         IF(pb1) res = 8;//(RESULT: 8)MR / VFO
         IF(pb0) res = 9;//(RESULT: 9)SPLIT
         
         k8 = 0; k1 = 1; delay_us(ondelay);
         IF(pb1) res = 10;//(RESULT: 11)VFO < > M
         k8 = 0; k4 = 0; k2 = 0; k1 = 0;
...
int8 buttons(INT1 option)
{
   ...
   IF(pb2)
   {
      ...
      else
      {
         count = 0;
         btnres = scan_buttons();
         if(btnres) {btn_down = 1; return 50;}
      ...
      } else 
      {
         mic_count = 0;
         micres = scan_mic_buttons();
         if(micres) {mic_down = 1; return 50;}
      }
...
void program_offset()
{
   ...
   WHILE(true)
   {
      ...
      IF(btnres == 1)
      {
         ...
         delay_ms(1000);
         VFD_data(0xFF, 0xFF, frequency, 0xFF, 0,0,1,0);
         delay_ms(1000);
         btnres = 0;
         res = 1;
         GOTO start;
...
void update_screen(int8 state, int1 force)
{
switch(state)
      {
         case 4: if(!cl) {VFD_data (0xFF, dcs, cb_channel, 0xFF, 0,0, 6, 0);} else {VFD_data (0xFF,dcs, frequency, 0xFF, 0,0, 0, 0);} break;
         default: VFD_data (get_state(0),dcs, frequency, mem_channel, 0,0, fine_tune_display, 0); break;
      }
      if(force) send_disp_buf (0);
