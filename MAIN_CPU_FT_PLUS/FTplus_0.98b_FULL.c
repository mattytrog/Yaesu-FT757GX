...
//0.98b Changelog
//FEATURE: PTT VIA CAT. Very slight, easy modification and PTT via CAT will work natively (in Kenwood mode only!). Ideal for WSJT-X etc
//BUGFIX: Hamlib will work now. Corrected sending 36 characters rather than 37
//BUGFIX: If cycling between VFO A and VFO B rapidly, frequencies could become mixed up. This was due to the cached frequency taking too long to get saved to VFO "slot". Fixed.
//IMPROVEMENTS: BCD counter logic redone CORRECTLY. Rather than activating C,B and A separately, its done at PORT level. This needed flipping around the data toggles as natively, they are backwards.
...
'A', 'I', 3, 1,  //AI0 = SET. NO ANSWER. IF off. AI1 = SET. ANSWER. IF on. Answer IF
...
switch (res) {
//all no answer
case 1: if(buffer[2] == '0') AI = 0;
        if(buffer[2] == '1') AI = 1;
        break;
//case 2: AI = 1; break;
case 3: down_button(); break;
case 4: up_button(); break;
case 5: if(active_vfo == 0) temp_value = frequency;
        if(active_vfo == 1) temp_value = load_vfo_f(0);
        cat_read_freq(temp_value, 5);
        send_cat_ans(0); 
        calc_if();
        break;

case 6: if(active_vfo == 1) temp_value = frequency;
        if(active_vfo == 0) temp_value = load_vfo_f(1);
        cat_read_freq(temp_value, 29);
        send_cat_ans(1); 
        calc_if();
        break;

case 7: temp_value = cat_set_freq(5); 
        if (active_vfo == 0) 
         {
            frequency = temp_value; save_cache_vfo_f(frequency); save_vfo_f(0, temp_value);
         } else save_vfo_f(0, temp_value);
         send_cat_ans(0); 
         calc_if();
         break;
case 8: temp_value = cat_set_freq(29);
        if (active_vfo == 1) 
         {
            frequency = temp_value; save_cache_vfo_f(frequency); save_vfo_f(1, temp_value);
...
case 10: calc_if(); send_if(); break;
case 11: send_cat_ans(2); break; //LK;
case 12: lock_dial_kenwood(buffer[2]); break; //LK+0 or 1;
case 15: dummy_mode = (buffer[2]); save_dummy_mode_n(dummy_mode); break;
...
case 22: cat_tx_request = 0; cat_transmit(cat_tx_request); break; //set rx mode
case 23: cat_tx_request = 1; cat_transmit(cat_tx_request); break; //set tx mode
...
if((cat_tx_request) || (cat_tx_transmitting))
{
      ...
      if((cat_tx_request) && (!cat_tx_transmitting)) 
      {
         if((!sl) && (!cl))
         {
         if(mem_mode != 1) save_cache_vfo_f(frequency);
      ...
      if((!cat_tx_request) && (cat_tx_transmitting)) 
      {
         ...
         if((!sl) && (!cl))
         {
         if(mem_mode != 1) frequency = load_cache_vfo_f();
         else frequency = load_cache_mem_mode_f();
         }
      }
}
