//Yaesu CPU firmware v0.95b (c)2025
...
const char testhelp[]= {
"\n\rWelcome to FTplus serial help \r\n\n"
"Yaesu stock commands: XX = number not relevant.  \r\n\n"
"XXXXXXXX01 - Split on/off \r\n"
"XXXXXXXX02 - MR/VFO mode \r\n"
"XXXXXXXX03 - VFO>M \r\n"
"XXXXXXXX04 - Dial lock \r\n"
"XXXXXXXX05 - VFO A/B \r\n"
"XXXXXXXX06 - M>VFO \r\n"
"XXXXXXXX07 - Up button (500k/band/memory - depending on VFO / MR / 500k button \r\n"
"XXXXXXXX08 - Down button (500k/band/memory - depending on VFO / MR / 500k button \r\n"
"000000000A - Frequency LSB first. eg 674523010A would be 12.345.67 \r\n"
"XXXXXXXX0B - M>VFO \r\n\n"
"Enhanced commands: XX = number not relevant.\r\n\n"
"00000000E0 - 00000000EE - Fast memory save. LSB first. eg 67452301E0 = 12.345.67 to ch 0, 67452301EE = 12.345.67 to ch 14(E)\r\n"
"00000000FB - Dial speeds in hz * 10 in accelerated dial mode (0-255 each speed). LSB = slowest(speed1).\r\n"
"  example 050A1932FB would be: Speed1 = 05, Speed2 = 0A(10), Speed3 = 19(25), Speed4 = 32(50)"
"  giving Speed1 50hz, Speed2 100hz, Speed3 250hz, Speed4 500hz\r\n"
"XXXXXXXXFC - Temporary wideband (until next power off)\r\n"
"XXXXXXXXFD - CB mode on/off \r\n"
"XXXXXXXXFE - Reset EEPROM \r\n\n"
"Easy Commands (write frequency normally) \r\n\n"
"000000000F - Frequency eg 123456780F = 12.345.678 hz \r\n"
"0000000010 - 000000001E - Fast memory save. Last byte is channel, so eg 1234567810 would save 12.345.678 to channel 0. 123456781E would save 12.345.678 to channel E(14); \r\n"
"  example 0700000019 would save 7.000.00 to channel 9. Remember to start with 0 if you require under 10 mhz!\r\n"
};

int32 frequency, vfo_frequency, mr_frequency, cb_frequency;
int32 disp_checksum;
int8 mem_channel, PLLband, band, mem_mode, dcs, speed_dial, dhz, speed1, speed2, speed3, speed4, mic_pressed, res1, res2, COMMAND_LENGTH;
int8 d1 = 0,d2 = 0,d3= 0,d4= 0,d5= 0,d6= 0,d7= 0,d8= 0,d9= 0,d10= 0, dummy_mode = 1, dialres = 0;
      ...
      while(kbhit())
         {
               ...
               buffer[next_in] = temp_byte;
               ++next_in;
               if(temp_byte == 0x3B) valid = 1;
               if(next_in > (COMMAND_LENGTH)) 
               {
                  while(kbhit()) 
                  {
                     delay_ms(10);getc();
         ...
         if((valid) && (buffer[next_in - 1] == 0x3B))
         {
         command_received = 1;
         bytes_used = next_in;
         next_in = 0;
...
static int8 t1, t2, t3, t4, t5, t6, t7, t8;
static int32 old_temp_value;
int32 temp_value = frequency;
int i;

if(temp_value != old_temp_value)
{
...
ifbuf[11] = 48 + t7; //Hz10
for (i = 12; i < 26; i++) {ifbuf[i] = ('0');}
//!ifbuf[12] = ('0'); //Hz1
//!ifbuf[13] = ('0'); //StepFrq (N/A)
//!ifbuf[14] = ('0'); //StepFrq (N/A)
//!ifbuf[15] = ('0'); //StepFrq (N/A) 
//!ifbuf[16] = ('0'); //StepFrq (N/A)
//!ifbuf[17] = ('0'); //StepFrq (N/A)
//!ifbuf[18] = ('0'); //RIT +/-
//!ifbuf[19] = ('0'); //RIT hz 1000
//!ifbuf[20] = ('0'); //RIT hz 100
//!ifbuf[21] = ('0'); //RIT hz 10
//!ifbuf[22] = ('0'); //RIT hz 1
//!ifbuf[23] = ('0'); //RIT on/off
//!ifbuf[24] = ('0'); //XIT On/off (N/A)
//!ifbuf[25] = ('0'); //Mem Bank (N/A)
...
ifbuf[29] = 48 + dummy_mode; //Mode 1 = LSB, 2 = USB, 3 = CW, 4 = FM, 5 = AM, 6 = FSK, 7 = CWN. All dummy values. Should reflect mode change in application
if(mem_mode == 0)
   {
      if(active_vfo == 0) ifbuf[30] = ('0'); //VFO0 (VFO A)
      if(active_vfo == 1) ifbuf[30] = ('1'); //VFO1 (VFO B)
   }
   else if(mem_mode == 1) ifbuf[30] = ('2'); //MR Mode
ifbuf[31] = ('0'); //Scan (0 = off, 1 = on)
if(sl) ifbuf[32] = ('1'); else ifbuf[32] = ('0'); //Split (0 = off, 1 = on)
for (i = 33; i < 37; i++) {ifbuf[i] = ('0');}
//!ifbuf[33] = ('0'); //Tone (N/A)
//!ifbuf[34] = ('0'); //Tone Freq digit 1 (N/A)
//!ifbuf[35] = ('0'); //Tone Freq digit 2 (N/A)
//!ifbuf[36] = ('0'); //Offset (+/-) (N/A)
ifbuf[37] = (';'); //Terminator
...
void send_freq(int32 temp_value)
{
int8 t1, t2, t3, t4, t5, t6, t7, t8;
split_value(temp_value, t1, t2, t3, t4, t5, t6, t7);
if(t1 == 15) putc('0'); else putc(48 + t1);
if(t2 == 15) putc('0'); else putc(48 + t2);
if(t3 == 15) putc('0'); else putc(48 + t3);
if(t4 == 15) putc('0'); else putc(48 + t4);
if(t5 == 15) putc('0'); else putc(48 + t5);
if(t6 == 15) putc('0'); else putc(48 + t6);
if(t7 == 15) putc('0'); else putc(48 + t7);
putc('0');
putc(';');
}
...
else if(res == 5) {putc('F'); putc('A'); putc('0'); putc('0'); putc('0'); send_freq(load_vfo_f(0));}
else if(res == 6) {putc('F'); putc('B'); putc('0'); putc('0'); putc('0'); send_freq(load_vfo_f(1));}

else if(res == 7)
{
i1 = buffer[5] - 48; //10mhz
i2 = buffer[6] - 48; //1mhz
i3 = buffer[7] - 48; //100khz
i4 = buffer[8] - 48; //10khz
i5 = buffer[9] - 48; //1khz
i6 = buffer[10] - 48; //100hz
i7 = buffer[11] - 48;

temp_value = ((i1 * 1000000) + (i2 * 100000) + (i3 * 10000) + (i4 * 1000) + (i5 * 100) + (i6 * 10) + (i7));
if (active_vfo == 0) {frequency = temp_value; save_cache_vfo_f(temp_value);}
save_vfo_f(0, temp_value);
}
...
i5 = buffer[9] - 48; //1khz
i6 = buffer[10] - 48; //100hz
i7 = buffer[11] - 48; //10hz

temp_value = ((i1 * 1000000) + (i2 * 100000) + (i3 * 10000) + (i4 * 1000) + (i5 * 100) + (i6 * 10) + (i7));
if (active_vfo == 1) {frequency = temp_value; save_cache_vfo_f(temp_value);}
save_vfo_f(1, temp_value);
}

else if(res == 9)
{
if(buffer[2] == '0') {active_vfo = 0; mem_mode = 0; save_vfo_n (active_vfo); save_mode_n (mem_mode);}
if(buffer[2] == '1') {active_vfo = 1; mem_mode = 0; save_vfo_n (active_vfo); save_mode_n (mem_mode);}
if(buffer[2] == '2') {mem_mode = 1; save_vfo_n (active_vfo); save_mode_n (mem_mode);}
}

else if(res == 10) send_if();
else if(res == 15) dummy_mode = (buffer[2] - 48);
else if(res == 26) puts("ID006;");
