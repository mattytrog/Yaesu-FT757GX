...
//v1.3 Changelog
//FEATURE: VFO Button Tuning
//IMPROVEMENTS: Now using a display buffer. Saved 10%+ space in ROM
//BUGFIXES: KENWOOD EMULATION: PTT mode in Kenwood emulation sometimes wasn't triggered. This was due to wrong amount of commands being scanned and some switch/case errors
//Also TX works correctly up to 115200 baud now. VFO A and B are correct now hopefully and shouldn't overwrite each other. Timeout errors are hopefully dealt with now.
//BUGFIXES: Sending transceiver ID is hit or miss at the best of times. This requirement varies between apps. I haven't found any that actually NEED it, so it is disabled
//by default. If you need it, (all it does is send "ID006;"), then it can be enabled. Simply send IE1; over serial to enable. IE is <I>D <E>nable. To disable, send IE0;
...
int32 baud_rate;
int8 mem_channel, PLLband, band, mem_mode, dcs, speed_dial, dhz, speed1, speed2, speed3, speed4, mic_pressed, res1, res2, COMMAND_LENGTH, baud_rate_n, Q64_val, Q64_tmp;
...
{if(per_band_offset) save32 ((PLLband + 21), value); else save32 (20, value);}
int32 load_offset_f() 
{if(per_band_offset) return (load32 (PLLband + 21)); else return (load32 (20));}
...
{
#ifdef debug_cat_tx
if (tx_request == 1) {Q64(5); dl = 1; process_dcs(0);} else {Q64(0); dl = 0; process_dcs(0);}
#else 
...
void blink_vfo_display(int8 PA1, int8 PA2, int32 PA3, int8 PA4)
{
   ...
   {
      force_update = 1; 
      vfo_disp(PA1, PA2, PA3, (PA4 + 20));
      beep(); delay_ms(50);
      vfo_disp(PA1, PA2, PA3, PA4); delay_ms(150);
   }
}

void blink_vfd_data(int8 PA1, int8 PA2, int32 PA3, int8 PA4, int8 PA5)
{
   for (int i = 0; i < 2; i++)
   {
   force_update = 1;
   VFD_data(PA1, PA2, PA3, PA4, PA5);
   beep(); 
   delay_ms(50); 
   VFD_data(PA1, PA2, PA3, 0xFF, PA5);
   delay_ms(150);
   }
}
...
{ 
   ...
   if(!long_press_down)
      {
         ...
         {
            if(band == 0) band = 10;
            --band; frequency = band_bank[band];
         }
         else frequency -= 50000;
      }
      else
      {
         long_press_down = 0; if(mem_channel > 0) --mem_channel;
   ...
   {
      if(mem_channel > 0) --mem_channel;
      blink_vfd_data(0xFF, 0xFF, cb_channel, mem_channel, 2);
      long_press_down = 0;
   }
#endif
...
void up_button()
{
   ...
   if(!long_press_up)
      ...
      {
         long_press_up = 0; if(mem_channel < 14) ++mem_channel;
         blink_vfo_display(active_vfo, dcs, frequency, mem_channel); 
      }
   }
   if(mem_mode == 1)
...
{  int8 temp_mode = mem_mode;
   ...
   frequency = load_mem_ch_f(mem_channel);
   blink_vfo_display(0xFF, dcs, frequency, mem_channel);
...
void mvfo_button()
{
   ...
   blink_vfo_display(0xFF, dcs, load_mem_ch_f(mem_channel), mem_channel);
   }
   else
   {
   save_cache_vfo_f(cb_frequency);
   save_vfo_f(active_vfo, cb_frequency);
   blink_vfd_data(0xFF, 0xFF, cb_frequency, mem_channel, 0);
   } 
   mode_switch(temp_mode);
}
...
void vfom_swap_button()
{
   ...
   if(temp_mode == 0)
   {
   blink_vfo_display(0xFF, dcs, load_cache_mem_mode_f(), mem_channel);
   }
   if(temp_mode == 1)
   {
   blink_vfo_display(0xFF, dcs, load_vfo_f(active_vfo), mem_channel);
   }
   mode_switch(temp_mode);
...
void vfoab_button()
{
   if(mem_mode == 0)
   {
      if(active_vfo == 0)
      {
      save_vfo_f(0, frequency);
      active_vfo = 1;
      
      }
      else
      {
      save_vfo_f(1, frequency);
      active_vfo = 0;
...
{
force_update = 1;
if(cat_mode == 0) {VFD_data(0xFF, 0xFF, 1, 0xFF, 2); save_cat_mode_n (1); cat_mode = 1;}
else              {VFD_data(0xFF, 0xFF, 0, 0xFF, 2); save_cat_mode_n (0); cat_mode = 0;}
...
void flash_freq_data(int8 current, int1 startflash);

void manual_adjust_frequency()
   {
   if(mem_mode != 0) return;
   int32 temp_freq = 0;
   int8 curr_digit = 0;
   int8 btnres = 0;
   int8 tmp_refresh = 60;
   int8 refresh_count = 0;
   if(d3 == 15) d3 = 0;
   if(d4 == 15) d4 = 0;
   while(true)
   {
   ...
   btnres = buttons();
   if(btnres == 6) {flash_freq_data (curr_digit, 1); ++curr_digit;}
   if(curr_digit == 9) curr_digit = 1;
   if(curr_digit != 8)
   {
   if(btnres == 3) //up
      {
      flash_freq_data (curr_digit, 1);
      if(curr_digit == 1)
         {if(d3 < 9) d3+=1;}
      if(curr_digit == 2)
         {if(d4 < 9) d4+=1;}
   ...
   if(btnres == 2) //up
    ...
    //if(btnres == 23) {frequency +=10; split_value (frequency, d3, d4, d5, d6, d7, d8, d9); if (d3 == 15) d3 = 0;}
      if(curr_digit == 8)
      {
         k2 = 1;
         if((pb1) || (pb0))
         {
            temp_freq = (((int32)d3 * 1000000) + ((int32)d4 * 100000) + ((int32)d5 * 10000) + ((int32)d6 * 1000) + ((int32)d7 * 100) + ((int32)d8 * 10) + ((int32)d9));
            if(pb0) temp_freq +=50;
            if(pb1) temp_freq -=50;
            if(temp_freq > 3199990) temp_freq = 3199990;
            else if(temp_freq < 50000) temp_freq = 50000;
   ...
   if(btnres == 26) break;
   if(!refresh_count){
   if(!btnres) flash_freq_data (curr_digit, 0);
   }
   ++refresh_count;
   if(refresh_count == tmp_refresh) refresh_count = 0;
   
   int32 temp_freq = (((int32)d3 * 1000000) + ((int32)d4 * 100000) + ((int32)d5 * 10000) + ((int32)d6 * 1000) + ((int32)d7 * 100) + ((int32)d8 * 10) + ((int32)d9));
   if(temp_freq > 3199990) temp_freq = 3199990;
   else if(temp_freq < 50000) temp_freq = 50000;
   frequency = temp_freq;
   force_update = 0;
...
if(test)
{
   ...
   {
   testfreq = frequency + test;
   VFD_data(0xFF, 0xFF, test, 0xFF, 2);
   }
}
...
if(!dir)
{
if(btnres == 2) {res = 1; test -=1;}
else if(btnres == 3) {res = 1; test +=1;}
}
else
{
if(btnres == 2) {res = 1; test +=1;}
...
void send_disp_buf()
{
   ...
   for(int i = 4; i < 13; i++){newcheck += disp_buf[i];} if(newcheck == oldcheck) return;
   }
   ...
   for(int i = 0; i < 13; i++)
      ...
      if ((disp_buf[i] & 1)==1) k8=1;
      if ((disp_buf[i] & 2)==2) k4=1;
      if ((disp_buf[i] & 4)==4) k2=1;
      if ((disp_buf[i] & 8)==8) k1=1;
...
void VFD_data(int8 vfo_grid, int8 dcs_grid, int32 value, int8 channel_grid, int8 display_type)
{
   ...
   if(display_type == 2) {disp_buf[4] = 15; disp_buf[5] = 15; disp_buf[6] = 15; disp_buf[7] = 15; disp_buf[8] = d7; disp_buf[9] = d8; disp_buf[10] = d9; disp_buf[11] = 15;}
   if(display_type == 3) {disp_buf[4] = 15; disp_buf[5] = 15; disp_buf[6] = d7; disp_buf[7] = d8; disp_buf[8] = d9; disp_buf[9] = 15; disp_buf[10] = 15; disp_buf[11] = 15;}
   if(display_type == 5) {disp_buf[4] = 15; disp_buf[5] = 15; disp_buf[6] = 15; disp_buf[7] = 15; disp_buf[8] = 15; disp_buf[9] = 0; disp_buf[10] = 15; disp_buf[11] = 15;}
   
   disp_buf[12] = channel_grid;
   send_disp_buf();
}
...
void vfo_disp(int8 vfo, int8 dcs, int32 freq, int8 ch)
{
   ...
   VFD_data(v1, v2, freq, v10, fine_tune_display);
}
...
if(current == 1) if(!flash) disp_buf[6] = 15; else disp_buf[6] = d3;
if(current == 2) if(!flash) disp_buf[7] = 15; else disp_buf[7] = d4;
if(current == 3) if(!flash) disp_buf[8] = 15; else disp_buf[8] = d5;
if(current == 4) if(!flash) disp_buf[9] = 15; else disp_buf[9] = d6;
if(current == 5) if(!flash) disp_buf[10] = 15; else disp_buf[10] = d7;
if(current == 6) if(!flash) disp_buf[11] = 15; else disp_buf[11] = d8;
if(current == 7) if(!flash) {disp_buf[11] = 15;}
                 else {disp_buf[6] = d4; disp_buf[7] = d5; disp_buf[8] = d6; disp_buf[9] = d7; disp_buf[10] = d8; disp_buf[11] = d9;}
if(current == 8) if(!flash) {disp_buf[6] = 15; disp_buf[7] = 15; disp_buf[8] = 15;; disp_buf[9] = 15; disp_buf[10] = 15; disp_buf[11] = 15;}
                 else {disp_buf[6] = d3; disp_buf[7] = d4; disp_buf[8] = d5;; disp_buf[9] = d6; disp_buf[10] = d7; disp_buf[11] = d8;}
force_update = 1;
send_disp_buf();
...
int8 buttonaction (int8 res)
{
   ...
   if(res == 1) 
   {
                           ...
                           {
                           force_update = 1; VFD_data(0xFF, 0xFF, frequency, 0xFF, 0); delay_ms(1000);
                           }
   }
   else if(res == 9)
   {
   ...
   else if (res == 28)     {if(gen_tx) {beep(); toggle_cb_mode();}}
   else if (res == 29)     {if(gen_tx) {beep(); VFD_special_data(4); force_update = 1; VFD_data(0xFF, 0xFF, cb_channel, 0xFF, 2); cb_region = 2; save_cb_reg_n(cb_region);}}
#else
...
#ifdef store_to_eeprom
   else if  (res == 21)    {beep(); program_offset();}
   ...
   else if  (res == 25) 
   {
      ...
      reset_cpu();
                              ...
                              if(!dl)
                              {
                              beep(); toggle_fine_tune_display();
                              }
                              else
                              {
                              beep(); manual_adjust_frequency();
                              }
   }
...
int8 buttons()
{
      ...
      if(pb2)
      {
         ...
         if (pb0){while(pb0){++count;  delay_ms(100); if(count >=10) {long_press = 1; break;}} btn_down = 1; res = 6;  goto out;} //(RESULT: 6) Dial lock
...
void calc_if()
{
   ...
   if(d3 == 15) ifbuf[5] = 48 + 0; else ifbuf[5] = 48 + d3; //Mhz10
   if(d4 == 15) ifbuf[6] = 48 + 0; else ifbuf[6] = 48 + d4; //Mhz1
...
void cat_read_freq(int32 temp_value, int8 base)
{
      ...
      if(t1 == 15) cat_ans[base] = 48; else cat_ans[base] = (48 + t1);
      if(t2 == 15) cat_ans[base + 1] = 48; else cat_ans[base + 1] = (48 + t2);
      if(t3 == 15) cat_ans[base + 2] = 48; else cat_ans[base + 2] = (48 + t3);
      if(t4 == 15) cat_ans[base + 3] = 48; else cat_ans[base + 3] = (48 + t4);
      if(t5 == 15) cat_ans[base + 4] = 48; else cat_ans[base + 4] = (48 + t5);
      if(t6 == 15) cat_ans[base + 5] = 48; else cat_ans[base + 5] = (48 + t6);
      if(t7 == 15) cat_ans[base + 6] = 48; else cat_ans[base + 6] = (48 + t7);
}
...
int32 cat_set_freq(int8 base)
{
      ...
      i1 = cat_ans[base] - 48; //10mhz
      i2 = cat_ans[base + 1] - 48; //1mhz
      i3 = cat_ans[base + 2] - 48; //100khz
      i4 = cat_ans[base + 3] - 48; //10khz
      i5 = cat_ans[base + 4] - 48; //1khz
      i6 = cat_ans[base + 5] - 48; //100hz
      i7 = cat_ans[base + 6] - 48;
    
      
      
      int32 temp_value = ((i1 * 1000000) + (i2 * 100000) + (i3 * 10000) + (i4 * 1000) + (i5 * 100) + (i6 * 10) + (i7));
...
void FA_read()
{
int32 temp_value = load_vfo_f(0);
           cat_flush();
           cat_read_freq(temp_value, 5);
...
{
cat_flush();
int32 temp_value = cat_set_freq(5);
           if(active_vfo == 0){ save_cache_vfo_f(temp_value); save_vfo_f(0, temp_value); frequency = temp_value;}
           if(active_vfo == 1){ save_cache_vfo_f(temp_value); save_vfo_f(1, temp_value); frequency = temp_value;}
           FA_read();
}
...
if(sl)
   ...
   //if(tx_mode) cat_tx = 1; else cat_tx = 0;
}
