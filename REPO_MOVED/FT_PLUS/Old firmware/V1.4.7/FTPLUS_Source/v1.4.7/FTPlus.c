...
//Yaesu CPU firmware v1.4.7 (c)2025
...
void set_defaults()
{
   ...
   for (i = 52; i <= 53; i++)    {save32 (i, 0); }
                                 ...
                                 save_offset_f(0);
                                 save8(band1_n, calc_band(700000));
                                 save8(band2_n, calc_band(700000));
                                 save8(band3_n, calc_band(700000));
...
void load_values()
{
      ...
      band1 =           load8(band1_n);
      band2 =           load8(band2_n);
      band3 =           calc_band (load_mem_ch_f(mem_channel));
      #endif
...
#ELSE
      ...
      speed_dial =      0;
      dcs =             15;
      band1 =           calc_band (700000);
      band2 =           calc_band (700000);
      band3 =           calc_band (700000);
      savetimerON =     0;
      offset =          load_offset_f();
#ifdef include_cb
      cb_region =       0;
...
void main()
{
   ...
   while(true)
   {
      ...
      #ifdef include_cat
      if((!pause_cat) && (!cat_wait))
      {
           cat_update = check_cat();
           if(cat_update)
           {
           state = get_state();
           counter = 100; //move counter above button scan threshold, but below maximum, so we get some cache time rather than immediate saving
           }
      }
      ...
      if(!update)
      {
       ...
       if(counter > 24000)
       {
          if(cat_update) {cat_update = 0; counter = 100;}
          counterstart = 0;
          calc_all_bands(); //update all bands in case we have missed a band update
          if((!cl) && (!sl))
          {
