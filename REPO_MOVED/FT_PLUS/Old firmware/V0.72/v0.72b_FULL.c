//Yaesu CPU firmware. (c)2024 Matthew Bostock v0.71b

//!0.7 - 0.71 Changelog
//!Changed 100 and 10hz component. Now all PLL / BCD counter operations should be accurate
//!Added missing custom CAT commands back
...
void save_value(int8 base, int32 value)
{
//
...
void update_PLL(int32 calc_frequency)
{
      ...
      int32 offset = load_value(PLLband + 20);
...
void down_button()
{
   ...
   frequency = load_value(mem_channel + 2);
   }
#ifdef include_cb  
   if(cb_mode)
   {
...
void mvfo_button()
{
   ...
   {
   frequency = load_value(mem_channel + 2);
   save_value(active_vfo, frequency);
   }
   if(mr_mode)
   {
   save_value(active_vfo, frequency);
...
void vfom_button()
{
   if(vfo_mode)
   {
   save_value(mem_channel + 2, frequency);
   }
   if(mr_mode)
   {
...
void mrvfo_button()
{
   if(vfo_mode)
   {
      save_value(active_vfo, frequency); //save vfo
      frequency = load_value(mem_channel + 2);
...
void vfom_swap_button()
{
   if(vfo_mode)
   {
   frequency = load_value(active_vfo); //load vfo contents
   save_value(17, frequency); //save vfo to cache
   frequency = load_value(mem_channel + 2);//load mem channel
   save_value(active_vfo, frequency); //save mem channel to active vfo
   frequency = load_value(17); //load cached vfo
   save_value(mem_channel + 2, frequency); //save saved vfo to mem ch
   frequency = load_value(active_vfo); //load vfo contents
   return;
   }
...
void toggle_cb_mode()
{
   if(!cb_mode)
   {
   vfo_mode = 0; mr_mode = 0; cb_mode = 1;
   ...
   {
   vfo_mode = 1; mr_mode = 0; cb_mode = 0;
...
if(sl)
   ...
   if(cl)
   {
      if(tx_mode)
      {
         if(!ctx)
         {
            save_value(18, frequency);
            frequency = load_value(17);
            ctx = 1;
            update = 1;
         }         
      }
...
{
#ifdef include_offset_programming
save_value(PLLband + 20, 0);
update_PLL(frequency);
int32 test = load_value(PLLband + 20);
int8 res = 0;
int8 btnres = 0;
int32 testfreq = 0;
...
if(dir) save_value(PLLband + 20, (1000000 + test)); 
if(!dir) save_value(PLLband + 20, (test)); 
break;
}
}
#endif
...
void main()
{
   ...
   for (i = 19; i < 30; i++)
      {save_value(i, 0);}
   #endif
   if(gen_tx == 1 && cb_mode) {cb_mode = 0; mr_mode = 0; vfo_mode = 1;}
   frequency = load_value(active_vfo);
   if(frequency == 0) frequency = 710000;
   update = 1;
   dl = 0; cl = 0; sl = 0;
   if(vfo_mode || mr_mode) cb_mode = 0;
   ...
   if(update)
      ...
      if(cb_region == 0 || cb_region == 2) frequency = (cb_channel_bank[cb_channel - 1]);
      if(cb_region == 1) frequency = (cb_channel_bank[cb_channel + 39]);
      update_PLL(frequency);
      update = 0;
      count = 0;
      }
