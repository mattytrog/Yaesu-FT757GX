...
int32 frequency;
int8 mem_channel, PLLband, band, mem_mode, dcs, speed_dial, dhz, speed1, speed2, speed3, speed4;
...
#define dial_timer_max 40000 //max spin down-timer. All percentages are based of this. Default 1000
...
if(res == 1)
 ...
 if(res == 5)
   ...
   {
   errorbeep(2);
   if(mem_mode == 0) {mrvfo_button(); return;}
   #ifdef include_cb
   if(gen_tx)
   {
      if(mem_mode == 1) {toggle_cb_mode(); return;}
      if(mem_mode == 2) {toggle_cb_mode(); mrvfo_button(); return;}
   }
   #else
   if(mem_mode == 1) {mrvfo_button(); return;}
   #endif
   }
      ...
      if((freq1 == freq2) || (mem_mode == 2)) {errorbeep(2); pms = 0; return;}
            ...
            if(freq2 < freq1)
            {
               ...
               while(true)
               {
                  ...
                  if(sw_pms) break;
                  if(buttons() == 4) break;
                  if(mic_fast && mic_up && mic_dn) {while (mic_fast && mic_up && mic_dn){} break;}
                  if(stopped == 1)
                  {
                     if(!squelch_open) 
                     {
