//Yaesu CPU firmware v0.82b (c)2024
...
//!Program acceleration dial over serial
...
1400000, 1800000, 2100000, 2400000, 2800000, 3199990
};
...
int32 frequency;
int8 mem_channel, PLLband, band, mem_mode, dcs, speed_dial, dhz, speed1, speed2, speed3, speed4, dcount, mic_pressed, res1, res2;
...
void save_speed1_n(int8 res) {save8(5, res);}
int8 load_speed1_n() {return (load8(5));}

void save_speed2_n(int8 res) {save8(4, res);}
int8 load_speed2_n() {return (load8(4));}

void save_speed3_n(int8 res) {save8(3, res);}
int8 load_speed3_n() {return (load8(3));}

void save_speed4_n(int8 res) {save8(2, res);}
int8 load_speed4_n() {return (load8(2));}
...
{
if(mode == 0) {frequency = load_vfo_f(active_vfo); return;}
else if(mode == 1) {frequency = load_mem_ch_f(mem_channel); return;}
#ifdef include_cb
else if(mode == 2) 
   {
   if(cb_region == 0) {channel_amount = 40; frequency = cb_channel_bank[cb_channel - 1];}
   else if(cb_region == 1) {channel_amount = 40; frequency = cb_channel_bank[cb_channel + 39];}
   else if(cb_region == 2) {channel_amount = 80; frequency = cb_channel_bank[cb_channel - 1];}
   return;
   }
#endif
...
#define out_speed3 100     //Faster still
...
#define dial_timer_max 40000 //max spin down-timer. All percentages are based of this. Default 40000
...
#define scan_pause_countVFO 20000
#define scan_pause_countMR 200
...
void program_mem_scan()
{
   ...
   int8 limit = 0;
   int8 start = 0;
   int8 tmp_cb_channel = 0;
   if(mem_mode == 0) res = 1;
   if(mem_mode == 1) res = 2;
   if(mem_mode == 2) res = 3;
   if(sw_500k) res +=3;

   //res1/res4 band scan (VFO)
   //res2 between mem channels scan (VFO)
   //res3 CB scan (current region)
   //res5 saved mem channel scan
   //res6 80ch CB scan
   
   //VFO between band scan
   //freq1 - Start frequency, freq2 - end
   ...
   limit = 14; start = 0;
   if(mem_channel < limit) freq2 = load_mem_ch_f(mem_channel + 1);
   else freq2 = load_mem_ch_f(start);
   }
   if(freq1 == freq2) {errorbeep(2); pms = 0; return;}
   ...
   while(true)
      {
         ...
         else VFD_data(0xFF, 0xFF, tmpfreq1, 0xFF, 0);
         update_PLL(tmpfreq1);
         if(sw_pms) break;
         if((!squelch_open) && (!stopped))
         {
            if(freq1 > freq2) tmpfreq1-=1;
            if(freq2 > freq1) tmpfreq1+=1;
         } 
         else stopped = 1;
         ...
         if(stopped)
         {
            ...
            if(!dial_clk) 
            {
               if(!dial_dir) tmpfreq1-=10;
               else if(dial_dir) tmpfreq1+=10;
               if(freq1 < freq2)
               {
               if(tmpfreq1 < freq1) tmpfreq1 = freq1;
               else if(tmpfreq1 > freq2) tmpfreq1 = freq2;
               }
               else if(freq1 > freq2)
               {
               if(tmpfreq1 > freq1) tmpfreq1 = freq1;
         ...
         if(!stopped)
         {
         if(tmpfreq1 == tmpfreq2) {tmpfreq1 = freq1; tmpfreq2 = freq2;}
         }
         if(res == 0) break;
      }
   ...
   {
   freq1 = load_mem_ch_f(mem_channel);
   tmpfreq1 = freq1; start = 0; limit = 14;
   }
   ...
   while(true)
      {
         ...
         {
         #ifdef include_cb
            if((cb_region == 0) || (res == 6))
            {
            if(!blink) VFD_data(2, 0xFF, tmp_cb_channel+1, 0xFF, 2);
            else VFD_data(0xFF, 0xFF, tmp_cb_channel+1, 0xFF, 2);
            }
            else
            {
            if(!blink) VFD_data(2, 0xFF, tmp_cb_channel-39, 0xFF, 2);
            else VFD_data(0xFF, 0xFF, tmp_cb_channel-39, 0xFF, 2);
            }
            #endif
         }
         update_PLL(tmpfreq1);
         if(res == 5) delay_ms(200);
         if((res == 3) || (res == 6)) delay_ms(20);
         if(sw_pms) break;
          if((!squelch_open) && (!stopped))
          {
         ...
         {
            ++counter2;
            if(counter2 > scan_pause_countMR) 
            {
               counter2 = 0; 
               stopped = 0;
            }
