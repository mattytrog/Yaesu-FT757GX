...
#define slow_speed_threshold 230
...
#define negative_pulse_sample_rate 10 //how often to check for negative pulse to fight against the decrement. Higher number means easier acceleration. Lower is harder (eg faster and harder). Default 50
...
#define dial_timer_pullback 4 //dial increment when negative (eg when slowing down spinning). How hard we fight against the decrement. Needs to be more than decrement. Default 20. Lower = acceleration increments kick in earlier
...
int8 freq_dial_basic(int32 &value, int8 dial_increment)
{
   ...
   else
   {
         ...
         res2 = res1;
         if(dial_increment == 1) return 1; else return 2;
      }
...
int8 update_encoder_tuning(int8 state)
{
   int8 res = 0;
#ifdef include_cb   
      if(((state == 4) && (!cl))) res = misc_dial8 (cb_channel, 0);
      else
#else
      if(state != 4)
#endif
      {
         IF (speed_dial == 0)
         {
            ...
            switch(fine_tune)
            {
            //1 revolution is 256 pulses. 1 revolution must equal 10khz (1000 in rig). 1000/256 = 3.9, 
            //so 3.9 increment. 4 is too fast, so we go for 3
            //case 0: update = (freq_dial_basic (frequency, 2)); break;
            
            case 0: res = (freq_dial_basic (temp_freq, speed1)); break;
            case 1: res = (freq_dial_basic (temp_freq, 1));break;
         ...
         IF (speed_dial == 1)
         {
            ...
            switch(fine_tune)
            {
            case 0: res = (freq_dial_accel_type1 (temp_freq, speed1)); break;
            case 1: res = (freq_dial_accel_type1 (temp_freq, 1));break;
            }
            storage_buffer[state] = temp_freq;
         }
#endif       
