...
INT8 curr_mem_ch, res, limit;
   ...
   if (state == 1)
   {
      ...
      freq1 = band_bank[band1];
      IF (band1 < 10)
      {
      freq2 = band_bank[band1+1];
      }
      else 
      {
      freq2 = band_bank[0];
   ...
   if (state == 2)
   {
      ...
      freq1 = band_bank[band2];
      IF (band2 < 10)
      {
      freq2 = band_bank[band2+1];
      }
      else 
      {
      freq2 = band_bank[0];
   ...
   while(true)
   {
      ...
      if (((state >= 1) && (state <= 3)) || ((state >= 5) && (state <= 6)))
      {
         switch(flash)
         {
            case 1: VFD_data (vgrid, 0xFF, curr_freq, 0xFF, 0,0,0, 0) ; break;
            case 0:  VFD_data (0xFF, 0xFF, curr_freq, 0xFF, 0,0,0, 0) ; break;
         }
      }
 #ifdef include_cb     
      if((state == 4) || (state == 8))
      {
         ...
         if(!cl)
         {
            switch(flash)
            {
               case 1: IF (flash) VFD_data (3, 0xFF, curr_mem_ch, 0xFF, 0,0, 6, 0); break;
               case 0: VFD_data (0xFF, 0xFF, curr_mem_ch, 0xFF, 0,0, 6, 0);  break; 
            }
         }
         else
         {
            switch(flash)
            {
               case 1: VFD_data (0xFF, dcs, curr_freq, 0xFF, 0,0,0, 0) ; break;
               case 0:  VFD_data (0xFF, 0xFF, curr_freq, 0xFF, 0,0,0, 0) ; break;
            }
         }
      ...
      if(state == 7)
      {
         ...
         switch(flash)
         {
            case 1: VFD_data (3, 0xFF, curr_freq, curr_mem_ch, 0,0,0, 0) ; break;
            case 0: VFD_data (0xFF, 0xFF, curr_freq, curr_mem_ch, 0,0,0,0) ; break;
         }
   ...
   if(squelch_open)
   {
      storage_buffer[get_state()] = curr_freq;
   #ifdef include_cb
