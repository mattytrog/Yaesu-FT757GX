...
clear_interrupt(INT_RDA);
   ...
   int32 cat_set_freq(INT8 base)
   {
      ...
      if((temp_value < min_freq) || (temp_value > max_freq)) {temp_value = storage_buffer[get_state()];  cat_flush(0);}
      RETURN temp_value;
   }
...
int8 check_cat()
   {
         ...
         if(pb2)
         {
            ...
            {
            while(pb2){}
            IF (cat_mode == 0) catres = parse_cat_command_yaesu ();
            IF (cat_mode == 1) catres = parse_cat_command_kenwood ();
            }
